...
TEST(DpdkConfigParamTest, SetPortParam) {
  ...
  port_params->set_type(PORT_TYPE_VHOST);
...
TEST(DpdkConfigParamTest, HotplugConfig) {
  ...
  hotplug_config->set_qemu_hotplug(HOTPLUG_ADD);
  ...
  ASSERT_TRUE(config.SetHotplugParam(PARAM_HOTPLUG, sport).ok());
