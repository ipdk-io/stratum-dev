...
::util::Status DpdkChassisManager::SetHotplugParam(
  ...
  if (config.HasAllOf(GNMI_CONFIG_HOTPLUG_REQUIRED) &&
      hotplug_params->qemu_hotplug == HOTPLUG_ADD) {
    if (!config.port_done) {
    ...
    RETURN_IF_ERROR(HotplugPortHelper(node_id, unit, sdk_port_id,
    ...
    config.Remove(GNMI_CONFIG_HOTPLUG_VAL);
    hotplug_params->qemu_hotplug = NO_HOTPLUG;
  } else if (config.HasAnyOf(GNMI_CONFIG_HOTPLUG_VAL) &&
             hotplug_params->qemu_hotplug == HOTPLUG_DEL) {
    if (!config.hotplug_done) {
      config.Remove(GNMI_CONFIG_HOTPLUG_VAL);
      return MAKE_ERROR(ERR_INTERNAL)
