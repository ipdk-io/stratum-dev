...
class DpdkHalTest : public ::testing::Test {
 protected:
  ...
  static ::testing::StrictMock<AuthPolicyCheckerMock>*
      auth_policy_checker_mock_;
  static DpdkHal* hal_;  // points to the singleton instance
};

constexpr char DpdkHalTest::kChassisConfigTemplate[];
constexpr char DpdkHalTest::kForwardingPipelineConfigsTemplate[];
...
::testing::StrictMock<AuthPolicyCheckerMock>*
    DpdkHalTest::auth_policy_checker_mock_ = nullptr;
DpdkHal* DpdkHalTest::hal_ = nullptr;

TEST_F(DpdkHalTest, SanityCheckFailureWhenExtURLsNotGiven) {
  FLAGS_external_stratum_urls = "";
...
TEST_F(DpdkHalTest, SanityCheckFailureWhenExtURLsAreInvalid) {
...
TEST_F(DpdkHalTest, SanityCheckFailureWhenPersistentConfigDirFlagNotGiven) {
...
TEST_F(DpdkHalTest, ColdbootSetupSuccessForNoSavedConfigAtAll) {
...
TEST_F(DpdkHalTest, ColdbootSetupSuccessForNoForwardingPipelineConfig) {
...
TEST_F(DpdkHalTest, ColdbootSetupSuccessForNoChassisConfig) {
...
TEST_F(DpdkHalTest, WarmbootbootSetupFailureForNoSavedConfig) {
...
TEST_F(DpdkHalTest, ColdbootTeardownSuccess) {
...
TEST_F(DpdkHalTest, ColdbootTeardownFailureWhenSwitchInterfaceShutdownFails) {
...
TEST_F(DpdkHalTest, ColdbootTeardownFailureWhenAuthPolicyCheckerShutdownFails) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
...
TEST_F(DpdkHalTest, WarmbootTeardownSuccess) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(Return(::util::OkStatus()));
...
TEST_F(DpdkHalTest, WarmbootTeardownFailure) {
  EXPECT_CALL(*switch_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
...
void* TestShutdownThread(void* arg) {
  ...
  static_cast<DpdkHal*>(arg)->HandleSignal(SIGINT);
  return nullptr;
}
