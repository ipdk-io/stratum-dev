...
::util::Status DpdkPortConfig::SetHotplugParam(
  SWBackendHotplugParams param_type, const SingletonPort& singleton_port) {
  ...
  switch (param_type) {
    ...
    case PARAM_HOTPLUG:
      params_set |= GNMI_CONFIG_HOTPLUG_VAL;
      hotplug.qemu_hotplug = params.qemu_hotplug();
      LOG(INFO) << "HotplugParam::qemu_hotplug = " << hotplug.qemu_hotplug;
      break;

    case PARAM_VM_MAC:
      params_set |= GNMI_CONFIG_HOTPLUG_VM_MAC;
