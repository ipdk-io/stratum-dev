...
void SignalRcvCallback(int value) {
  ...
  write(Es2kHal::pipe_write_fd_, &value, sizeof(value));
  errno = saved_errno;
}
...
Es2kHal* Es2kHal::singleton_ = nullptr;
ABSL_CONST_INIT absl::Mutex Es2kHal::init_lock_(absl::kConstInit);
int Es2kHal::pipe_read_fd_ = -1;
...
::util::Status Es2kHal::SanityCheck() {
...
void Es2kHal::HandleSignal(int value) {
...
::util::Status Es2kHal::RegisterSignalHandlers() {
...
::util::Status Es2kHal::UnregisterSignalHandlers() {
...
void* Es2kHal::SignalWaiterThreadFunc(void*) {
  int signal_value;
  int ret = read(Es2kHal::pipe_read_fd_, &signal_value, sizeof(signal_value));
  ...
  Es2kHal* hal = Es2kHal::GetSingleton();
