...
::util::Status Es2kSwitch::PushChassisConfig(const ChassisConfig& config) {
  ...
  for (const auto& entry : node_id_to_device_id) {
    ...
    ASSIGN_OR_RETURN(auto* tdi_node, GetEs2kNodeFromDeviceId(device_id));
...
::util::Status Es2kSwitch::PushForwardingPipelineConfig(
  ...
  absl::WriterMutexLock l(&chassis_lock);
  ASSIGN_OR_RETURN(auto* tdi_node, GetEs2kNodeFromNodeId(node_id));
...
::util::Status Es2kSwitch::WriteForwardingEntries(
  ...
  ASSIGN_OR_RETURN(auto* tdi_node, GetEs2kNodeFromNodeId(req.device_id()));
...
::util::StatusOr<Es2kNode*> Es2kSwitch::GetEs2kNodeFromDeviceId(
    int device_id) const {
  Es2kNode* tdi_node = gtl::FindPtrOrNull(device_id_to_tdi_node_, device_id);
  if (tdi_node == nullptr) {
...
::util::StatusOr<Es2kNode*> Es2kSwitch::GetEs2kNodeFromNodeId(
  ...
  Es2kNode* tdi_node = gtl::FindPtrOrNull(node_id_to_tdi_node_, node_id);
  if (tdi_node == nullptr) {
    return MAKE_ERROR(ERR_INVALID_PARAM)
           << "Node with ID " << node_id
