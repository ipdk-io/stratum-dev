...
class Es2kHalTest : public ::testing::Test {
 protected:
  ...
  static void SetUpTestCase() {
    // Set up Es2kHal class instance under test.
  ...
  static ::testing::StrictMock<AuthPolicyCheckerMock>*
      auth_policy_checker_mock_;
  static Es2kHal* hal_;  // points to the singleton instance
};

constexpr char Es2kHalTest::kChassisConfigTemplate[];
constexpr char Es2kHalTest::kForwardingPipelineConfigsTemplate[];
...
::testing::StrictMock<AuthPolicyCheckerMock>*
    Es2kHalTest::auth_policy_checker_mock_ = nullptr;
Es2kHal* Es2kHalTest::hal_ = nullptr;

TEST_F(Es2kHalTest, SanityCheckFailureWhenExtURLsNotGiven) {
  FLAGS_external_stratum_urls = "";
...
TEST_F(Es2kHalTest, SanityCheckFailureWhenExtURLsAreInvalid) {
...
TEST_F(Es2kHalTest, SanityCheckFailureWhenPersistentConfigDirFlagNotGiven) {
...
TEST_F(Es2kHalTest, ColdbootSetupSuccessForSavedConfigs) {
...
TEST_F(Es2kHalTest, ColdbootSetupSuccessForNoSavedConfigAtAll) {
...
TEST_F(Es2kHalTest, ColdbootSetupSuccessForNoForwardingPipelineConfig) {
...
TEST_F(Es2kHalTest, ColdbootSetupSuccessForNoChassisConfig) {
...
TEST_F(Es2kHalTest, ColdbootSetupFailureWhenChassisConfigPushFails) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(Es2kHalTest, WarmbootbootSetupFailureForNoSavedConfig) {
...
TEST_F(Es2kHalTest, WarmbootSetupFailureWhenUnfreezeFails) {
  // Setup and save the test config(s).
  ChassisConfig chassis_config;
  ForwardingPipelineConfigs forwarding_pipeline_configs;
  FillTestChassisConfigAndSave(&chassis_config);
...
TEST_F(Es2kHalTest, ColdbootTeardownSuccess) {
...
TEST_F(Es2kHalTest, ColdbootTeardownFailureWhenSwitchInterfaceShutdownFails) {
...
TEST_F(Es2kHalTest, ColdbootTeardownFailureWhenAuthPolicyCheckerShutdownFails) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
...
TEST_F(Es2kHalTest, WarmbootTeardownSuccess) {
  EXPECT_CALL(*switch_mock_, Shutdown()).WillOnce(Return(::util::OkStatus()));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
      .WillOnce(Return(::util::OkStatus()));
...
TEST_F(Es2kHalTest, WarmbootTeardownFailure) {
  EXPECT_CALL(*switch_mock_, Shutdown())
      .WillOnce(
          Return(::util::Status(StratumErrorSpace(), ERR_INTERNAL, kErrorMsg)));
  EXPECT_CALL(*auth_policy_checker_mock_, Shutdown())
...
void* TestShutdownThread(void* arg) {
  ...
  static_cast<Es2kHal*>(arg)->HandleSignal(SIGINT);
  return nullptr;
}
