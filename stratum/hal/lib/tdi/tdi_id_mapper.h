...
class TdiIdMapper {
 public:
  ...
  ::util::StatusOr<tdi_id_t> GetActionSelectorTdiRtId(
      tdi_id_t action_profile_id) const ABSL_LOCKS_EXCLUDED(lock_);

  // Gets the action profile ID of an action selector.
  ::util::StatusOr<tdi_id_t> GetActionProfileTdiRtId(
      tdi_id_t action_selector_id) const ABSL_LOCKS_EXCLUDED(lock_);
 ...
 private:
  ...
  absl::flat_hash_map<tdi_id_t, uint32> tdi_to_p4info_id_ ABSL_GUARDED_BY(lock_);
  absl::flat_hash_map<uint32, tdi_id_t> p4info_to_tdi_id_ ABSL_GUARDED_BY(lock_);
