...
class TofinoChassisManager {
 ...
 private:
  ...
  ::util::Status UnregisterEventWriters() ABSL_LOCKS_EXCLUDED(chassis_lock);
  ...
  std::shared_ptr<Channel<TdiPortManager::PortStatusEvent>>
      port_status_event_channel_ ABSL_GUARDED_BY(chassis_lock);
  ...
  std::map<int, uint64> device_to_node_id_ ABSL_GUARDED_BY(chassis_lock);

  // Map from node ID to device number.
  std::map<uint64, int> node_id_to_device_ ABSL_GUARDED_BY(chassis_lock);
  ...
  std::map<uint64, std::map<uint32, absl::Time>>
      node_id_to_port_id_to_time_last_changed_ ABSL_GUARDED_BY(chassis_lock);
  ...
  std::map<uint64, std::map<uint32, PortKey>>
      node_id_to_port_id_to_singleton_port_key_ ABSL_GUARDED_BY(chassis_lock);
  ...
  std::map<uint64, TofinoConfig::DeflectOnPacketDropConfig>
      node_id_to_deflect_on_drop_config_ ABSL_GUARDED_BY(chassis_lock);
