...
class Phal : public PhalInterface {
 public:
  ...
  // the instance.
  static Phal* CreateSingleton() ABSL_LOCKS_EXCLUDED(config_lock_, init_lock_);
 ...
 private:
  ...
  static Phal* singleton_ ABSL_GUARDED_BY(init_lock_);
  ...
  // Owned by this class.
  std::unique_ptr<AttributeDatabase> database_ ABSL_GUARDED_BY(config_lock_);

  // Owned by this class.
  std::unique_ptr<SfpAdapter> sfp_adapter_ ABSL_GUARDED_BY(config_lock_);

  // Owned by this class.
  std::unique_ptr<OpticsAdapter> optics_adapter_ ABSL_GUARDED_BY(config_lock_);

  // Store backend interfaces for later Shutdown. Not owned by this class.
  std::vector<PhalBackendInterface*> phal_interfaces_ ABSL_GUARDED_BY(config_lock_);
};
